...
class Mbv3SmallFast(nn.Module):
    ...
    def load_pretrain(self):
        checkpoint = model_zoo.load_url("{}/{}".format(_MODEL_URL_DOMAIN,_MODEL_URL_SMALL))
...
class DBFace(nn.Module):
    def __init__(self, has_landmark=False, wide=24, has_ext=False, upmode="UCBA"):
        ...
        self.hmap = nn.Conv2d(in_channels=wide, out_channels=2, kernel_size=1)
        self.regs = nn.Conv2d(in_channels=wide, out_channels=2, kernel_size=1)
        self.w_h_ = nn.Conv2d(in_channels=wide, out_channels=2, kernel_size=1)
...
def get_pose_net(num_layers=18, head_conv=64, num_classes=20):
  ...
  model = DBFace(has_landmark=False, wide=64, has_ext=False, upmode="UCBA")
  return model