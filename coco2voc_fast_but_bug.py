...
src_base = os.path.join('/workspace/DATA/zhatu/zhatu0814/coco')
dst_base = os.path.join('/workspace/DATA/zhatu/zhatu0814/voc')
...
sets = {
    "train": os.path.join(src_base, "train.json"),
    "val": os.path.join(src_base, "val.json"),
}
...
for stage, filename in sets.items():
    ...
    for im in tqdm(data["images"], "Parse Images"):
        ...
        images[im["id"]] = img
        img_names.add(im['file_name'])
    for img_name in tqdm(img_names):
        shutil.copy(src_base + '/image/' + img_name, dst_base + '/JPEGImages')
